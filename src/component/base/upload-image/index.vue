...
export default {
  ...
  data() {
    return {
      ...
      tempremoteFuc: this.remoteFuc, // 重写父组件的自定义上传方法
    }
  },
  props: {
    /** 每一项宽度 */
  ...
  watch: {
    ...
    itemList:{
      handler(newList){
        this.$emit('parentFun',this.itemList)
      },
    }
  },
  mounted() {
    this.initItemList(this.value)
    this.tempremoteFuc = function(item) {
      const that = this
      const cos = new COS({
        SecretId: 'AKIDFAzXrwPP3UcTAeTlaAWdT3NQTRN1xrth',
        SecretKey: 'kMhld06we7KGwRgeOzKfjywdBPN8bpoa',
      })
      cos.putObject(
        {
          Bucket: 'zb2l3-cms-1302782490',
          Region: 'ap-beijing',
          Key: item.localSrc.split('/').pop(),
          StorageClass: 'STANDARD',
          // Body: fs.createReadStream(item.localSrc), // 上传文件对象
          Body: item.file,
        ...
        function (err, data) {
          ...
          that.$emit('uploadFinal',{id:item.id,uploadinfo:data})
        },
      )
    }
  },
  methods: {
    ...
    async uploadImg(item) {
      ...
      if (this.tempremoteFuc && typeof this.tempremoteFuc === 'function') {
        const result = await new Promise(resolve => {
          let remoteFucResult
          try {
            remoteFucResult = this.tempremoteFuc(item, remoteData => {
